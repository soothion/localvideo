...
class pc_base {
	//FTP上传
    	public static function ftp_upload($file,$ftp_server,$ftp_user_name,$ftp_user_pass){
            ...
            $target=$pathinfo['basename'];
             // upload a file 
            ftp_put($conn_id, $target, $file, FTP_BINARY);
...
class local_video
{
    ...
    function convert()
    {
        ...
        if ($verifyToken == $this->options['token'])
        {
            ...
            $mp4      = $this->ffmpeg . ' -i  "'  . $orgFile . '" -vcodec libx264 -movflags faststart -strict -2 -g 1 -keyint_min 2 '.$setting.' ' .$mp4File;
            exec($mp4);
